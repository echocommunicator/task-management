...
-- Password for new demo users: Welcome@123
-- ============================================================
...
BEGIN
  ...
  IF v_admin_id IS NULL THEN
    RAISE EXCEPTION 'User a@in-sync.co.in must sign up first before seeding demo data.';
  END IF;
  ...
  -- ===================== CREATE TEAM USERS =====================
  -- Only create the 4 additional demo users (admin already exists)
  ...
  SELECT t.id, v_admin_id, 'Great progress on this. Lets aim to close it by EOD.', 'comment', v_org_id, NOW() - INTERVAL '2 days'
